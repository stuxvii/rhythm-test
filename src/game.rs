...
pub fn draw_ui(mut d: RaylibDrawHandle<'_>, app_state: &AppState) {
    if let Some(song_data) = &app_state.song_state.song_data {
        ...
        if let Some(last_note) = song_data.notes.last() {
            ...
            design::draw_text(&mut d, &song_data.name, Align::End, Align::Middle, app_state.ui.note_height, app_state.ui.fg, offset, &app_state.ui);
            design::draw_text(&mut d, &text_rem_time, Align::End, Align::End, app_state.ui.note_height, app_state.ui.fg, offset, &app_state.ui);
            design::draw_text(&mut d, &text_cur_time, Align::End, Align::Start, app_state.ui.note_height, app_state.ui.fg, offset, &app_state.ui);
        }
        ...
        let misses: Vec<&Note> = song_data.notes.iter().filter(|n| n.state == Judgment::Miss).collect();
...
pub fn game_loop(mut d: RaylibDrawHandle<'_>, mut app_state: &mut AppState, current_tap: &Option<Sound<'_>>, current_song: &Option<Music<'_>>) {
